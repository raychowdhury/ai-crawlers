...
const cookieName='crawler_workspace', ttl=30*60*1000;
...
export function publicState(session){
  ...
  return {csrf:session.csrf,expiresAt:session.expiresAt,providers,connection:session.connection?{provider:'github',repository:session.connection.repository,base:session.connection.base}:null,
...
export function createWorkspace({origin,audit,acquireAudit,connect=connectGithub,prepare=prepareGithubPlan,submit=submitGithubPlan,recover=recoverGithubPlan,fetchPublic=safeFetch,now=Date.now}={}) {
  const sessions=new Map(), acquire=createAuditGate({maxActive:8,perClient:40,globalLimit:100});
  const cleanup=()=>{for(const [key,s] of sessions)if(s.expiresAt<=now()) {s.connection=null;s.plan=null;sessions.delete(key);}};
    ...
    try {
      ...
      if(req.method==='GET' && route==='/api/workspace'){
        if(!session){
          ...
          if(sessions.size>=256)throw new ConnectionError('Too many sessions. Please try again later.',429);
          const id=secret();session={csrf:secret(),expiresAt:now()+ttl,connection:null,plan:null,history:[],busy:false};sessions.set(id,session);
      ...
      try {
        ...
        if(route==='/api/workspace/connect') {
